...
| Profile update | removed | `customer-profile-form.tsx` via `putAuth()` | `PUT /customers/me/profile` | cookie | customer | **migrated** | Frontend retargeted to NestJS; guarded 3-op `$transaction`, validation feedback, and empty-default-skip behavior preserved. Obsolete Next.js profile mutation removed. |
